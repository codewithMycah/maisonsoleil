...
const Navbar = () => {
  ...
  return (
    <nav className='bg-sun-50 relative h-24 p-6 flex items-center justify-between border-b-2 border-b-neutral-400 z-20 md:w-96 md:border-r-2 md:h-screen md:items-start md:border-b-0'>
      ...
      {isOpen && (
        <div className='bg-background md:hidden h-[calc(100dvh-84px)] absolute inset-0 mt-24 px-6 py-4 text-neutral-700'>
          <div className='h-full flex flex-col justify-between'>
            <div className='flex flex-col items-start gap-2'>
              {tabs.map((tab, index) => (
                <div key={index} className={`w-full flex flex-row items-center justify-between gap-2 p-4 hover:bg-neutral-0 hover:rounded-lg hover:ring-1 hover:ring-offset-1 hover:ring-offset-terracotta-400 hover:cursor-pointer font-medium ${tab.active === true ? 'bg-neutral-0 rounded-lg' : ''}`}>
                <div className='flex gap-2'>
                  {tab.icon}
                  {tab.label}
                </div>
                {tab.active === true && (
                  <span className='rounded-full bg-terracotta-700 w-6 h-6 text-center text-neutral-0'>1</span>
                )}
              </div>
              ))}
            </div>
      ...
      <div className='bg-background hidden md:block h-[calc(100dvh-84px)] absolute inset-0 mt-24 mx-6 py-4 text-neutral-700 border-t-2'>
        <div className='h-full flex flex-col justify-between'>
          <div className='flex flex-col items-start gap-2'>
            {tabs.map((tab, index) => ( 
              <div key={index} className={`w-full flex flex-row items-center justify-between gap-2 p-4 hover:bg-neutral-0 hover:rounded-lg hover:ring-1 hover:ring-offset-1 hover:ring-offset-terracotta-400 hover:cursor-pointer font-medium ${tab.active === true ? 'bg-neutral-0 rounded-lg' : ''}`}>
